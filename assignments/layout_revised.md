...
All of your layout rules should be executed within the constructor for your Part3View.
...
- Finally, your images should scale proportionately to fill their respective columns. This means that you must take the aspect ratio into account, so that as your images expand to fill the column, their height scales up as well, preserving their aspect ratio. Additionally, when rotated, the images should change width (and therefore height) to fit the larger columns.
...
For this part, you will have to make use of the LayoutInflater object to convert your part4_grid XML file into a view object.

LayoutInflaters are generated from a Context object, which is in this case passed in. From there, you can make use of the [LayoutInflater.inflate()](https://developer.android.com/reference/android/view/LayoutInflater.html#inflate(int,%20android.view.ViewGroup,%20boolean)) function, using a reference to your desired XML layout file, to convert an XML file into an accessible View object.

The setContentView() function we have been using within onCreate thus far actually does this internally, inflating the passed layout file to convert it into a View object in order to display it on the device.

If you imagine the XML layout as a tree-like structure, then we are following each branch to a set of element nodes, which we can convert into Java objects before attaching them to their parent view objects, repeating the process all the way up back to the root. The result of this operation is that we have transformed our (mostly) human-readable XML layouts into highly functional objects, which we can now attach our images to.
